...
agent = Agent(
    name="PG2 Backup Agent",
    description="Handles backup and restore for PostgreSQL Server 2",
    instructions="Use pgBackRest or appropriate scripts to perform backups and restores.",
    role="backup_agent",
    goal="Perform backup and restore of PG2 database",
    backstory="Responsible for maintaining PG2 backups safely",
)
...
class BackupRestoreAgent2:
    def __init__(self):
        self.name = "agent2"
        self.stanza = "pg2_17"
        self.data_dir = "/var/lib/postgresql/17/pg2_17"  # PG2 data directory
        self.port = 5434
    ...
    def perform_restore(self, backup_name=None, db_name=None, recent=False):
        ...
        if db_name:
            ...
            if recent:
                ...
                # Correctly handle backup_name even if it comes from user input parsing
        ...
        # -------------------------
        # Cluster-level restore (unchanged)
        # -------------------------
        else:
